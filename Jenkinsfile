pipeline {
    ...
    stages{
       ...
       stage('Docker Build and Deploy Snapshot') {
            steps {
                script {
                    def curlCmd = "curl -s \"https://api.adoptopenjdk.net/v3/assets/feature_releases/8/ga?architecture=x64&image_type=jdk&jvm_impl=hotspot&os=linux&page_size=1\" | grep -oP 'release_name'"
