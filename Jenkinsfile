pipeline {
    ...
    stages{
       ...
       stage('Docker Build and Deploy Snapshot') {
            steps {
                script {
                    def req="curl -s https://api.adoptopenjdk.net/v3/assets/feature_releases/8/ga?architecture=x64&heap_size=normal&image_type=jdk&jvm_impl=hotspot&os=linux&page=0&page_size=1&project=jdk&sort_order=DESC&vendor=adoptopenjdk"
                    def curlCmd = req+ "| grep -oP '\"semver\": \"\\K(.*)(?=\")'"
